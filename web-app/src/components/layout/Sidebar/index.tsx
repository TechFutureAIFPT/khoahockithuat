...
import { Home, FileText, Sliders, Upload, Sparkles, LayoutDashboard } from "lucide-react";
...
const STEPS: {
  ...
  desc: string;
    ...
    { key: "dashboard", icon: LayoutDashboard, label: "Thống kê", desc: "Bước 5/4" },
  ];
      ...
      const load = async () => {
        try {
          ...
          const unsub = onAuthStateChanged(auth, async (user) => {
            if (!user) return;
            setUserName(user.displayName || user.email?.split("@")[0] || userEmail.split("@")[0]);
            
            // Priority 1: Firebase standard photoURL
            if (user.photoURL) {
              setUserAvatar(user.photoURL);
            } else {
              // Priority 2: Custom Firestore avatar
              try {
                ...
                if (profile?.avatar) {
                  ...
                  setUserAvatar(localStorage.getItem(`avatar_${user.email || userEmail}`));
                }
              } catch {
                setUserAvatar(localStorage.getItem(`avatar_${user.email || userEmail}`));
              }
      ...
      return onLoginRequest ? (
        <div className={`p-3 border-t border-white/5 ${collapsed ? "px-2" : ""}`}>
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 h-10 rounded-xl bg-gradient-to-r from-cyan-600 to-indigo-600 hover:brightness-110 text-white font-bold transition-all ${collapsed ? "px-0" : "text-[13px]"}`}
          >
    ...
    return (
      ...
      >
        <button
          ...
          className={`w-full flex items-center rounded-xl hover:bg-slate-800/60 transition-colors group ${collapsed ? "justify-center p-2" : "gap-3 px-3 py-2.5"}`}
          title={collapsed ? userName || userEmail : undefined}
        >
          <div
            className={`rounded-lg flex items-center justify-center text-white text-[12px] font-bold flex-shrink-0 overflow-hidden border border-white/10 ${!userAvatar ? avatarColor : ""} ${collapsed ? "w-9 h-9" : "w-9 h-9"}`}
          >
            {userAvatar ? (
              <img
                src={userAvatar}
        ...
        {showMenu && (
          <div
            className={`absolute bottom-full mb-2 bg-slate-900 border border-white/8 rounded-xl shadow-2xl shadow-black/50 z-50 ${collapsed ? "left-full ml-2 w-48" : "left-3 right-3"}`}
          >
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const isStepEnabled = (step: AppStep): boolean => {
    ...
    // Always allow going back to any step that is already in completedSteps
    if (completedSteps.includes(step)) return true;

    // Progression logic
    if (step === "weights") return completedSteps.includes("jd");
    if (step === "upload") return completedSteps.includes("weights");
    if (step === "analysis") return completedSteps.includes("upload");
    if (step === "dashboard") return completedSteps.includes("analysis");
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`flex items-center h-[60px] border-b border-white/5 flex-shrink-0 ${collapsed ? "px-0 justify-center" : "px-5 gap-3"}`}
        >
          {collapsed ? (
            <button
            ...
            <>
              <div className="w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 border border-white/10 shadow-lg">
                <img
                  src="/images/logos/logo.jpg"
                  alt="Support HR"
                  className="w-full h-full object-contain"
            ...
            return (
              ...
              >
                ...
                {!collapsed && (
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-[13px] font-semibold leading-tight truncate ${isActive
                          ? "text-white"
                          : isEnabled
                            ? "text-slate-300"
                            : "text-slate-600"
                    ...
                    <p
                      className={`text-[11px] mt-0.5 ${isActive ? "text-slate-400" : "text-slate-600"}`}
                    >
                      {step.desc}
                    </p>
                  </div>
