  ...
  return (
    <div className={`relative ${className}`}>
      {isMenuItem ? (
        ...
        >
          <i className="fa-solid fa-layer-group text-[11px] text-slate-400 w-4 text-center" />
          <span className="text-[11px] text-slate-300 font-medium flex-1">
            Mẫu JD
          </span>
          {isOpen && <i className="fa-solid fa-chevron-right text-[8px] text-slate-600" />}
        </button>
      ) : (
        <button
          onClick={() => setIsOpen(!isOpen)}
      ...
      {isOpen && (
        <>
          ...
          />
          <div className={`absolute z-50 bg-slate-900 border border-slate-700/60 rounded-xl shadow-2xl shadow-black/40 overflow-hidden ${
            isMenuItem 
              ? "bottom-[10%] left-full ml-4 w-64" 
