      ...
      <div className={activeStep === "weights" ? "block h-screen" : "hidden"}>
        <Suspense fallback={<ModuleLoader />}>
          <WeightsConfig
            ...
            sidebarCollapsed={props.sidebarCollapsed}
            onComplete={async () => {
              // Auto-save template when weight configuration is complete
              if (props.uid && props.jdText.length >= 50 && props.jobPosition.length >= 3) {
                try {
                  const timestamp = new Date().toLocaleString('vi-VN');
                  await JDTemplateService.saveTemplate({
                    uid: props.uid,
                    name: `Auto: ${props.jobPosition} (${timestamp})`,
                    jdText: props.jdText,
                    jobPosition: props.jobPosition,
                    hardFilters: props.hardFilters,
                    weights: props.weights
                  });
                  console.log("✅ Auto-saved JD template successfully");
                } catch (err) {
