...
const GOOGLE_CLOUD_VISION_API_KEY = (import.meta as any)?.env
...
let geminiVisionModel: ReturnType<
  GoogleGenerativeAI["getGenerativeModel"]
> | null = null;
let activeVisionKeyIndex = 0;
let visionKeyWarningShown = false;

const getGeminiVisionModel = (): ReturnType<
  GoogleGenerativeAI["getGenerativeModel"]
> | null => {
  if (!geminiVisionKeys.length) {
    if (!visionKeyWarningShown) {
      console.warn(
        "[OCR] Không tìm thấy VITE_GEMINI_API_KEY_x để sử dụng fallback OCR bằng Gemini Vision.",
      );
  ...
  if (!geminiVisionClient) {
    geminiVisionClient = new GoogleGenerativeAI(
      geminiVisionKeys[activeVisionKeyIndex],
    );
    geminiVisionModel = geminiVisionClient.getGenerativeModel({
...
const runGoogleCloudVisionOCR = async (
  canvas: HTMLCanvasElement,
): Promise<string> => {
...
const detectCVSections = (text: string): Record<string, string> => {
  ...
  const sectionPatterns = {
    personal:
      /^(thông tin cá nhân|personal info|liên hệ|contact|hồ sơ|profile)/i,
...
export const parseCVFields = async (text: string): Promise<ParsedCV> => {
  ...
  result.name = extractField(
    sections,
    ["name", "full name", "họ tên", "tên"],
    (value) => ({
  ...
  result.phone = extractField(
    sections,
    ["phone", "tel", "mobile", "điện thoại", "sđt"],
    (value) => {
  ...
  result.location = extractField(
    sections,
    ["location", "address", "địa chỉ", "địa điểm"],
    (value) => ({
  ...
  result.experience = extractField(
    sections,
  ...
  result.education = extractField(
    sections,
    ["education", "học vấn", "học vấn", "bằng cấp"],
    (value) => ({
...
const detectSections = (text: string): Record<string, string> => {
  ...
  for (const line of lines) {
    ...
    if (
      lowerLine.match(
        /^(name|email|phone|location|experience|education|skills|kinh nghiệm|học vấn|kỹ năng|họ tên|điện thoại|địa chỉ)/i,
      )
    ) {
      // Save previous section
      if (currentSection && currentContent.length > 0) {
 ...
 */
const isTextContentSufficient = (
  text: string,
  minLength: number = MIN_PDF_TEXT_LENGTH,
): boolean => {
...
const performOptimizedOCR = async (
  ...
  try {
    ...
    if (GOOGLE_CLOUD_VISION_API_KEY) {
      console.log(
        "🚀 [OCR] Đang sử dụng Google Cloud Vision API (High Accuracy)...",
      );
      ...
      console.warn(
        "⚠️ [OCR] Google Cloud Vision trả về ít dữ liệu, chuyển sang Gemini Vision...",
      );
    }
    ...
    if (geminiVisionKeys.length > 0) {
      console.log(
        "🚀 [OCR] Đang sử dụng Gemini Vision làm công cụ chính để tối ưu độ chính xác...",
      );
      ...
      console.warn(
        "⚠️ [OCR] Gemini Vision trả về ít dữ liệu, kích hoạt Tesseract dự phòng...",
      );
    } else {
      console.log(
        "ℹ️ [OCR] Không tìm thấy Gemini Key, sử dụng Tesseract thuần.",
      );
    }
      ...
      options: {
        ...
        ...(documentType === "cv" && {
          tessedit_char_whitelist:
            "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ (),./\\-+&:;@%",
    ...
    for (let i = 0; i < attempts.length; i++) {
      try {
        const result = await Tesseract.recognize(canvas, attempts[i].lang, {
          ...
          logger: (m) => {
            if (m.status === "recognizing text") {
              console.log(
                `OCR Attempt ${i + 1} Progress: ${Math.round(m.progress * 100)}%`,
              );
            }
          },
        });
        ...
        if (
          ...
          result.data.text.length > bestResult.length * 0.8
        ) {
    ...
    if (
      (trimmedResult.length < 150 || bestConfidence < 80) &&
      geminiVisionKeys.length > 0
    ) {
      // Chỉ thử lại nếu kết quả Tesseract quá tệ
      console.log(
        "⚠️ [OCR] Tesseract kết quả kém, thử lại Gemini Vision lần cuối...",
      );
      const visionText = await runGeminiVisionOCR(canvas, documentType);
...
export const extractTextFromFile = async (
  ...
  if (file.size > FILE_SIZE_LIMIT_MB * 1024 * 1024) {
    throw new Error(
      `File is too large. Maximum size is ${FILE_SIZE_LIMIT_MB}MB.`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Lỗi xử lý file ${file.name}: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
};
