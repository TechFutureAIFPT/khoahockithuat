    ...
    const result = await apiQueue.add(async () => {
      for (let attempt = 0; attempt < apiKeys.length; attempt++) {
        try {
          ...
          const modelInstance = aiInstance.getGenerativeModel({
            model: model,
          ...
          console.warn(
            `API key ${currentKeyIndex + 1}/${apiKeys.length} failed [${isQuota ? "QUOTA" : status || "ERROR"}]:`,
            error?.message || error,
          );
          switchToNextKey();
        }
      ...
      throw new Error(
        "Tất cả API keys Gemini đã hết quota hoặc không hợp lệ. Vui lòng thêm API key mới vào file .env.local (VITE_GEMINI_API_KEY_5=...) hoặc kiểm tra quota tại https://aistudio.google.com",
      );
    });
...
const detailedScoreSchema = {
  type: SchemaType.OBJECT,
  properties: {
    "Tiêu chí": { type: SchemaType.STRING },
    Điểm: {
      type: SchemaType.STRING,
      description:
...
const analysisSchema = {
  ...
  items: {
    ...
    properties: {
      ...
      detectedLocation: { type: SchemaType.STRING },
      analysis: {
        type: SchemaType.OBJECT,
        properties: {
          "Tổng điểm": { type: SchemaType.INTEGER },
          Hạng: { type: SchemaType.STRING },
          "Chi tiết": {
            type: SchemaType.ARRAY,
...
Trả về JSON với các trường đã trích xuất và chuyển đổi:`;
  ...
  try {
    ...
    if (hardFilters.contractType) {
      const ct = hardFilters.contractSchemaType.trim();
